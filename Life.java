...
public class Life extends JPanel implements ActionListener, MouseListener, MouseMotionListener{
    ...
    private final int GRID_SIZE = 100; // Number of squares long each part of the board.
                                       // (Should not be less than 10 or more than 200).
    private boolean[][] alive; // Represents the board, alive[r][c] is true if the cells in row r and column c is alive.
    ...
    private JButton randomButton; // Button for filling the board randomly with each square having 25%
    ...
    public Life() {
        ...
        int cellSize = 600/GRID_SIZE; // Aim for about a 600-by-600 pixels board.
    ...
    private void doFrame() { // Compute the new state of the life board.
        ...
        for (int r = 0; r < GRID_SIZE; r++) {
            ...
            int left, right; //columns considered left nd right of column c.
            above = r > 0 ? r - 1 : GRID_SIZE-1;
            below = r < GRID_SIZE-1 ? r + 1 : 0;
            for (int c = 0; c < GRID_SIZE; c++) {
                left = c > 0 ? c - 1 : GRID_SIZE-1;
                right = c < GRID_SIZE-1 ? c + 1 : 0;
                int n = 0; // Number of alive cells in the 8 neighbouring cells.
                if (alive[above][left])
                    n++;
                if (alive[above][c])
                    n++;
    ...
    /**
     * Respond to Action events from one of the control buttons or from the timer.
     */
    public void actionPerformed(ActionEvent e) {
        ...
        if (src == quitButton) { // End the program.
        ...
        else if (src == stopGoButton) { // Start or stop the game, depending on whether or not it is currently running.
            if (timer.isRunning()) { // If the game is currently running, stop it.
                ...
                stopGoButton.setText("Start"); // Change text of button to "Start", since it can be used to start game.
            }
        }
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if (row >= 0 && row < display.getRowCount() && col >= 0 && col < display.getRowCount()) {
